...
public class TestB2AdvancedWidgetsX012 extends ViewTest {
    ...
    public void checkAppTheme() {
        String resName = "Theme.ColorGameX";
        int resId = 0;

        try {
            resId = activity.getPackageManager().getActivityInfo(activity.getComponentName(), 0).getThemeResource();
            assertNotEquals("Application theme should be " + resName,0,resId);
        } catch (Exception e) {
